...
var rootCmd = &cobra.Command{
	Use:          "api-gateway",
	Short:        "GoTaskFlow REST/gRPC API Gateway",
	Long:         "Exposes REST and gRPC endpoints for submitting and querying tasks.",
	SilenceUsage: true,
}

// Execute is the entry point called from cmd/api-gateway/main.go.
func Execute() {
...
func init() {
	...
	rootCmd.PersistentFlags().String("postgres-dsn",
		"postgres://taskflow:taskflow@localhost:5432/taskflow?sslmode=disable",
		"PostgreSQL DSN")

	bindFlag("log_level", rootCmd.PersistentFlags(), "log-level")
	bindFlag("postgres_dsn", rootCmd.PersistentFlags(), "postgres-dsn")

	rootCmd.AddCommand(serveCmd)
	rootCmd.AddCommand(newInitCmd("api-gateway", defaultAPIGatewayYAML))
